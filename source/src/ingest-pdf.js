...
export async function ingestPdf(buffer, options = {}) {
  ...
  if (useEmbedding) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (usePg) {
        ...
        if (embeddingStr !== null && (canStoreEmbeddingInDb || !embeddingStoreProbed)) {
          try {
            const { rows } = await db.query(
              'INSERT INTO public.documents (company_id, content, metadata, embedding) VALUES ($1, $2, $3, $4::vector) RETURNING id',
              [companyId, content, JSON.stringify(metadata), embeddingStr]
            );
        ...
        const { rows } = await db.query(
          'INSERT INTO public.documents (company_id, content, metadata) VALUES ($1, $2, $3) RETURNING id',
          [companyId, content, JSON.stringify(metadata)]
        );
        if (rows[0]?.id) ids.push(rows[0].id);
  ...
  const metadata = { source: 'pdf', filename, pages: numpages };
  if (usePg) {
    const storeStart = Date.now();
    const { rows } = await db.query(
      'INSERT INTO public.documents (company_id, content, metadata) VALUES ($1, $2, $3) RETURNING id',
      [companyId, trimmed, JSON.stringify(metadata)]
    );
    const elapsed = Date.now() - startTime;
    ...
    console.log('='.repeat(60) + '\n');
    return { ok: true, id: rows[0]?.id, pages: numpages };
  }
  ...
  console.log(`[ingest-pdf] ▶ FINISH  |  ${elapsed}ms total  |  ${numpages} pages → 1 row (no chunking)`);
  console.log('='.repeat(60) + '\n');
  return { ok: true, id: data?.id, pages: numpages };
}
